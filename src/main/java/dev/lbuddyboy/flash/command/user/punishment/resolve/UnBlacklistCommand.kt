...
@CommandAlias("unblacklist|removeblacklist")
@CommandPermission("flash.command.unblacklist")
object UnBlacklistCommand : BaseCommand() {
    @Default
    @CommandCompletion("@target")
    fun unBanIp(sender: CommandSender, @Name("target") uuid: UUID?, @Name("reason {-p}") reason: String) {
        ...
        if (punishment == null) {
            sender.sendMessage(
                CC.translate(
                    ...
                    "%FORMAT%",
                    PunishmentType.BLACKLIST.format
                )
            )
            return
        }
