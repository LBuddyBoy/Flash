...
@CommandAlias("silence|mute")
@CommandPermission("flash.command.mute")
public class MuteCommand extends BaseCommand {
    ...
    public static void mute(CommandSender sender, @Name("target") UUID uuid, @Name("duration") String time, @Name("reason {-p}") String reason) {
        ...
        if (user.hasActivePunishment(PunishmentType.MUTE)) {
            sender.sendMessage(CC.translate(FlashLanguage.PUNISHMENTS_ALREADY_PUNISHED.getString(), "%FORMAT%", PunishmentType.MUTE.getFormat()));
            return;
        }

        Punishment punishment = new Punishment(UUID.randomUUID(), PunishmentType.MUTE, uuid, senderUUID, System.currentTimeMillis(), duration, reason, FlashLanguage.SERVER_NAME.getString(), !isPub);
