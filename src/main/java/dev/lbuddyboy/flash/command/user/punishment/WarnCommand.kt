...
public class WarnCommand extends BaseCommand {
    ...
    public static void mute(CommandSender sender, @Name("target") UUID uuid, @Name("duration") @Default("perm") String time, @Name("reason {-p}") String reason) {
        ...
        if (user.hasActivePunishment(PunishmentType.WARN)) {
            sender.sendMessage(CC.translate(FlashLanguage.PUNISHMENTS_ALREADY_PUNISHED.getString(), "%FORMAT%", PunishmentType.WARN.getFormat()));
            return;
        }

        Punishment punishment = new Punishment(UUID.randomUUID(), PunishmentType.WARN, uuid, senderUUID, System.currentTimeMillis(), duration, reason, FlashLanguage.SERVER_NAME.getString(), !isPub);
        ...
        new GlobalMessagePacket(uuid, "&c&lYou have just been warned. &e&lReason: " + punishment.getSentFor()).send();
