...
@CommandAlias("ipban|ipbanish|ip-ban|banip")
@CommandPermission("flash.command.ipban")
public class IPBanCommand extends BaseCommand {

    @Default
    @CommandCompletion("@target")
    public static void ipBan(CommandSender sender, @Name("target") UUID uuid, @Name("duration") @Default("perm") String time, @Name("reason {-p}") String reason) {
        ...
        if (user.hasActivePunishment(PunishmentType.IP_BAN)) {
            sender.sendMessage(CC.translate(FlashLanguage.PUNISHMENTS_ALREADY_PUNISHED.getString(), "%FORMAT%", PunishmentType.IP_BAN.getFormat()));
            return;
        }

        Punishment punishment = new Punishment(UUID.randomUUID(), PunishmentType.IP_BAN, uuid, senderUUID, System.currentTimeMillis(), duration, reason, FlashLanguage.SERVER_NAME.getString(), !isPub);
